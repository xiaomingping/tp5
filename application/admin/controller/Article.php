...
class Article extends Allow
{
    ...
    {	
        ...
        $res = Db::name('articles')->where('title','like',"%".$k."%")->field('id,title,descr,pic,addtime')->paginate(2);
    ...
    public function postinsert(){
       ...
       //获取上传图片资源
        ...
        if(true !==$result){
            $this->error($result,"/adminarticle/add");
        }
        //  //移动到指定的目录下
        ...
        // //封装需要添加数据
        $data=$request->only(['title','descr','keywords','description']);
        ...
        if(Db::name("articles")->insert($data)){
            $this->success("添加成功","/adminarticle/index");
        }else{
            $this->error("添加失败");
        }
    }
    ...
    public function postdelete(){
        ...
        if(Db::name("articles")->where("id","{$id}")->delete()){
            //判断
            if(isset($array[1])){
            ...
            unlink($opic);
            //删除百度编辑器上传的图片
            $this->success("删除成功","/adminarticle/index");
        }else{
            $this->error("删除失败");
        }
       ...
       //获取上传图片资源
        ...
        if(empty($file)){
            if(DB::name('articles')->where('id',"{$id}")->update($data)){
                $this->success('修改成功','/adminarticle/index');
            }else{
                $this->error('修改失败');
            }
        }else{
        ...
        // ---------------------------------------------------------
            ...
            //执行修改
            if(DB::name('articles')->where('id',"{$id}")->update($data)){
                $this->success("修改成功","/adminarticle/index");die;
            }else{
                $this->error("修改失败");die;
            }
        }
