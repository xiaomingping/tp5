...
class Goods extends Allow
{
    ...
    {	
        ...
        $res = Db::name('goods')->where('title','like',"%".$k."%")->field('id,title,descr,pic,addtime')->paginate(2);
    ...
    public function postinsert(){
       ...
       //获取上传图片资源
        ...
        // //封装需要添加数据
        $data=$request->only(['title','descr','keywords','description','hshow']);
       ...
       //获取上传图片资源
        ...
        if(empty($file)){
            if(DB::name('goods')->where('id',"{$id}")->update($data)){
                $this->success('修改成功','/admingoods/index');
            }else{
                $this->error('修改失败');
            }
        }else{
        ...
        // ---------------------------------------------------------
            ...
            //执行修改
            if(DB::name('goods')->where('id',"{$id}")->update($data)){
                $this->success("修改成功","/admingoods/index");die;
            }else{
                $this->error("修改失败");die;
            }
        }
